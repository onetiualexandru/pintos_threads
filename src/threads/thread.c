...
// Return the state of the thread as a string
const char* get_thread_status_as_string(enum thread_status status){
...
thread_exit (void) 
{
  ...
  // afiseara thread cand iese
  //printf("Thread with tid = %d was created by parent with tid = %d and is DYING\n",thread_current()->tid,thread_current()->parent_tid);
   //printf("Thread %s tid = %d is DYING and it was created by parent with tid = %d\n",thread_current()->name,thread_current()->tid,thread_current()->parent_tid)
...
schedule (void) 
{
  ...
  if (cur != next){
    // ??? aici cred ca se intampla smecheria dar nu merge cum vreau eu pun un print si crapa tot trebuie sincronizat sau ceva si daca da cum ca efectiv nu inteleg?
    //PART 3. Thread switching (context switch). Thread state save and restore.
    //1. Display before and after each context switch in function schedule, in file “threads/thread.c”, the previous, current and next threads. Run the test “pintos run my_test_create_threads“ to see the result. WARNING: this may cause pintos to freeze due to thread corruptions caused by high level of recursion.
    prev = switch_threads (cur, next);
    //printf("tid=%d",prev->tid);
    /*printf("Schedule cur, next, prev --------");
...
allocate_tid (void) 
{
  ...
  //4. Change the allocate_tid function in file “threads/thread.c”, such that it only allocates even numbers for thread identifiers. Run the test above again to see the result.
